...
```svelte
<svg role="img" aria-label="Strength progression chart showing squat, bench, deadlift, and overhead press estimated 1RM over time">
  <title>Strength Progression (E1RM)</title>
  <desc>Multi-line chart with four lines representing different lifts, showing increasing strength over time with personal record markers.</desc>
  <!-- chart content -->
</svg>
```
...
<div use:inView on:enter={() => { chartVisible = true; loadChartData(); }}>
  {#if chartVisible}
